...
```c++
const int mod=1e9+7;const int N=2010;int c[N][N];
void init(){
    for(int i=0;i<N;i++){
        for(int j=0;j<=i;j++){
            if(!j) c[i][j]=1;
...
```c++
const int MAXN=200004;int fa[MAXN];int n;
void init(){
...
```c++
int sum[1000010]={0};int tree[1000010][26];int cnt=0;
void Insert(char s[]){
    int root=0;
    for(int i=0;s[i];i++)
    {
...
int main(){
    ...
    while (!pq.empty()){
        ...
        p[cur.v] = 1;
        dist[cur.v] = cur.w;
        for (auto &e : adj[cur.v]){if(!p[e.v]) pq.push({e.u, e.v, cur.w + e.w});}
    }
}
```
